...
//     const prompt = await this.promptGen(message, this.history);
//     const res = await this.resp(prompt);
//     return {"history":this.history}
//   }

//   async chatshowing(history): Promise<any> {
//     const data = history?.map((item) => {
//       return {
//         [item?.id?.[2]]: item?.kwargs?.content || '',
//       };
//     });
